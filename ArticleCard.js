...
export default function ArticleCard({ article, isTop, onPress }) {
    ...
    return (
      <TouchableOpacity style={styles.topCard} onPress={onPress} activeOpacity={0.85}>
        <View style={styles.topBadge}>
          <Text style={styles.topBadgeTxt}>今日头条</Text>
        </View>
        <View style={styles.topMeta}>
          ...
          <Text style={styles.timeTxt}>{timeAgo(article.created_at)}</Text>
        </View>
        <Text style={styles.topTitle}>{article.title}</Text>
        <Text style={styles.topSummary} numberOfLines={3}>{article.summary}</Text>
        ...
        <Text style={[styles.readMore, { color: catColor }]}>阅读全文 →</Text>
      </TouchableOpacity>
    );
  }
    ...
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.85}>
      ...
      {article.tags?.length > 0 && (
        <View style={styles.tags}>
          ...
          <Text style={[styles.readMoreSmall, { color: catColor }]}>阅读 →</Text>
        </View>
      )}
    </TouchableOpacity>
  );
...
const styles = StyleSheet.create({
  topCard: {
    ...
    backgroundColor: '#fff',
    borderRadius: 12,
    borderLeftWidth: 4, borderLeftColor: '#e11d27',
    shadowColor: '#000', shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.06, shadowRadius: 8, elevation: 3,
  },
  topBadge: {
    position: 'absolute', top: 12, right: 12,
    backgroundColor: '#e11d27', borderRadius: 4,
