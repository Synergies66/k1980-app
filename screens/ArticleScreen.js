...
import { View, Text, ScrollView, StyleSheet, ActivityIndicator, TouchableOpacity, Share, SafeAreaView } from 'react-native';
...
export default function ArticleScreen({ route }) {
  ...
  useEffect(() => {
    fetchArticleDetail(id).then(data => { setArticle(data); setLoading(false); });
  }, [id]);
    ...
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.meta}>
          ...
          <Text style={styles.time}>{new Date(article.created_at).toLocaleDateString('zh-CN')}</Text>
        ...
        <View style={styles.content}>{renderMarkdown(article.content)}</View>
        {article.tags?.length>0&&(
          <View style={styles.tags}>
            {article.tags.map(t=><View key={t} style={styles.tag}><Text style={styles.tagTxt}>{t}</Text></View>)}
          </View>
        )}
